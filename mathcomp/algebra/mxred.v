...
apply/(codiagonalisable_on _ mxdirect_eigenspaces) => // i/=.
  ...
  by rewrite commmx_stable_eigenspace.
...
Lemma codiagonalisableP n (fs : seq 'M[F]_n) :
  {in fs &, forall f g, commmx f g} /\ (forall f, f \in fs -> diagonalisable f)
  <-> codiagonalisable fs.
Proof.
split => [cdfs|[P Punit /allP/= fsD]]/=; last first.
  ...
  by rewrite /commmx -!conjmxM 1?diag_mxC// inE stablemx_unit ?unitmx_inv.
move: cdfs; rewrite (rwP (all_commmxP _)) => cdfs.
have [k] := ubnP (size fs); elim: k => [|k IHk]//= in n fs cdfs *.
case: fs cdfs => [|f fs]//=; first by exists 1%:M; rewrite ?unitmx1.
rewrite ltnS all_commmx_cons => -[/andP[/allP/(_ _ _)/eqP ffsC fsC dffs]] fsk.
...
have efg (i : 'I_(size rs)) g : g \in f :: fs -> stablemx (eigenspace f rs`_i) g.
   case: n => [|n'] in g f fs ffsC fsC {dffs rs1 fsk} * => g_ffs.
      by rewrite thinmx0 sub0mx.
  rewrite commmx_stable_eigenspace//.
...
apply: IHk; rewrite ?size_map/= ?ltnS//; split.
  apply/all_commmxP => _ _ /mapP[/= g gfs ->] /mapP[/= h hfs ->].
  rewrite -!conjmxM ?inE ?stablemx_row_base ?efg ?inE ?gfs ?hfs ?orbT//.
  by rewrite (all_commmxP _ fsC).
move=> _ /mapP[/= g gfs ->].
